  ...
  <div class="edit">
    <Card>
      ...
      <Form :model="naire" :label-width="120">
        <FormItem label="调研类别" v-show="loginUser().u_admin > '0'">
          <Select
            v-model="ntype"
            style="width: 260px"
            placeholder="请选择调研类别"
    ...
    <Card style="margin-top: 16px" v-if="false">
      <p slot="title">上传调研标准指标信息（可选）</p>
      <Form :label-width="120">
        <FormItem label="导入标准指标库">
          <Upload
            :action="`${baseURL}/naire/upload`"
            ...
            :on-error="handleError"
          >
            <Button icon="ios-paper">选择标准指标库文件（xls格式）</Button>
          </Upload>
        </FormItem>
        <FormItem label="调研指标比例">
          <Slider v-model="ratio" :tip-format="formatRatio"></Slider>
        </FormItem>
      </Form>
    </Card>
    ...
    <Row
      ...
      align="middle"
    >
      <Button style="margin-right: 16px" @click="handleSave">保存问卷</Button>
...
export default {
  ...
  created() {
    ...
    if (this.loginUser().u_admin === '0') {
      ...
      this.$store.commit('UPDATE_NTYPE', '');
      this.$store.commit('UPDATE_AREAS', '');
    }
  },
  watch: {
    // 如果路由有变化，会再次执行该方法
    ...
    changeType(id) {
      ...
      if (id === '0') {
        ...
        this.$http
          ...
          .then((response) => {
            this.$store.dispatch('clearQuestion');
            this.defaultQuestions = response.data.questionlist;
            this.$nextTick(() => {
      ...
      if (this.$route.params.id) {
        this.$store
          ...
          .then((response) => {
            ...
            this.$store.dispatch('updateNaire', response.data);
